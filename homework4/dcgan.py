...
def train():
    ...
    for epoch in range(flags["n_epoch"]):
        ...
        for i, (real_images, _) in enumerate(train_loader):
            ...
            fale_labels = torch.zeros(batch_size, device=device)

            # 真实图片通过判别器D，得到真实图片的判别结果
            d_real = D(real_images)
            loss_real = criterion(d_real, real_labels)

            # 随机生成噪声z，通过生成器G，得到生成图片G(z)
            z = torch.randn(batch_size, flags["z_dim"], device=device)
            fake_images = G(z)
            d_fake = D(fake_images.detach())  # detach()用于不计算梯度
            loss_fake = criterion(d_fake, fale_labels)
            ...
            # Generator
            # 计算生成器的损失
            d_fake_for_generator = D(fake_images) 
            g_loss = criterion(d_fake_for_generator, real_labels)
            # 计算生成器G的梯度，更新生成器G的参数
